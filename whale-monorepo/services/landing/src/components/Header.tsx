    ...
    <header className="fixed top-0 left-0 right-0 z-50 glass-nav transition-all duration-300 pt-[env(safe-area-inset-top,0)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-5 h-14 min-h-[3.5rem] flex items-center justify-between gap-3">
        <Link href="/" className="flex items-center gap-2 min-w-0" title="Sight Whale Home">
          <WhaleLogo className="h-8 w-8 flex-shrink-0 text-white drop-shadow-[0_0_15px_rgba(139,92,246,0.5)]" />
          <span className="font-bold tracking-tight text-base sm:text-lg text-white truncate">
        ...
        <nav className="hidden md:flex items-center gap-7">
          {navLinks.map(({ href, label }) => (
            <Link key={href} href={href} className="text-[13px] font-medium text-gray-300 hover:text-white transition-colors">
              {label}
            </Link>
          ))}
          <a href={TELEGRAM_DEEP_LINK_HEADER} target="_blank" rel="noopener noreferrer" className="text-[13px] font-medium text-gray-300 hover:text-white transition-colors flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <a href={CTA_URL} className="btn-primary text-xs px-4 py-2.5 min-h-[44px] min-w-[44px] inline-flex items-center justify-center shadow-[0_0_15px_rgba(139,92,246,0.2)] hover:shadow-[0_0_25px_rgba(139,92,246,0.4)] hidden sm:inline-flex">
            Get Alerts
          </a>
          <button
            type="button"
            onClick={() => setMobileOpen((o) => !o)}
            className="md:hidden p-2.5 -mr-2 min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded-lg text-gray-300 hover:text-white hover:bg-white/10 transition-colors"
      ...
      />
      <div
        className={`md:hidden fixed top-14 left-0 right-0 bottom-0 z-40 overflow-y-auto bg-[#0a0a0f] border-t border-white/10 transition-transform duration-200 ease-out ${
        ...
        <nav className="flex flex-col p-4 gap-1">
          {navLinks.map(({ href, label }) => (
            <Link
              ...
              href={href}
              onClick={() => setMobileOpen(false)}
              className="py-3.5 px-4 rounded-xl text-[15px] font-medium text-gray-200 hover:text-white hover:bg-white/10 active:bg-white/15 transition-colors min-h-[48px] flex items-center"
            >
              {label}
            </Link>
          ))}
          <a
            ...
            onClick={() => setMobileOpen(false)}
            className="py-3.5 px-4 rounded-xl text-[15px] font-medium text-gray-200 hover:text-white hover:bg-white/10 active:bg-white/15 transition-colors min-h-[48px] flex items-center gap-3"
          >
