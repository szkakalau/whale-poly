...
import { loadPublicHistorySignals, summarizeHistoryRows, summarizeHistoryByScoreTier } from '@/lib/history-signals';
import type { ScoreTierSummary } from '@/lib/history-signals';
...
import type { StarWhale } from '@/lib/whale-profiles';
...
const loadCachedScoreTiers = unstable_cache(
  async () => {
    const rows = await loadPublicHistorySignals(500);
    return summarizeHistoryByScoreTier(rows);
  },
  ['home-score-tiers-v2'],
  { revalidate: 300 },
);

const loadCachedStarWhale = unstable_cache(
  async () => {
    try { return await getStarWhale(); } catch { return null; }
  },
  ['home-star-whale-v2'],
  { revalidate: 300 },
);
...
async function ScorePerformanceSection() {
  let tiers: ScoreTierSummary[];
  try { tiers = await loadCachedScoreTiers(); } catch { return null; }
...
async function StarWhaleSection() {
  let whale: StarWhale | null;
  try { whale = await loadCachedStarWhale(); } catch { return null; }
