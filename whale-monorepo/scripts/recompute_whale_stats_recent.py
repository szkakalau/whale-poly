...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Recompute whale_stats for recently active wallets.")
    parser.add_argument("--hours", type=int, default=24, help="lookback hours for active wallets")
    parser.add_argument("--batch-size", type=int, default=200, help="wallets per batch")
    parser.add_argument("--raw-limit", type=int, default=50000, help="cap trades_raw rows per window")
    parser.add_argument("--history-limit", type=int, default=150000, help="cap whale_trade_history rows per window")
    parser.add_argument("--trade-cap", type=int, default=-1, help="max trades per wallet per window; 0=unlimited")
...
async def _fetch_raw_trades(session, since: datetime, raw_limit: int) -> list[TradeRaw]:
    q = select(TradeRaw).where(TradeRaw.timestamp >= since)
    if raw_limit > 0:
        q = q.order_by(TradeRaw.timestamp.desc()).limit(raw_limit)
    return (await session.execute(q)).scalars().all()


async def _fetch_history_rows(session, since: datetime, wallets: list[str], history_limit: int) -> list[WhaleTradeHistory]:
    q = (
...
async def main() -> int:
    ...
    raw_limit = max(int(args.raw_limit), 0)
    history_limit = max(int(args.history_limit), 0)

    if int(args.trade_cap) >= 0:
    ...
    since_active = now - timedelta(hours=hours)
    ...
    async with SessionLocal() as session:
        wallet_rows = (
            await session.execute(
                ...
                .where(WhaleTradeHistory.timestamp >= since_active)
                .distinct()
        ...
        if not wallets:
            ...
            return 0
        print(
            f"wallets={len(wallets)} hours={hours} batch_size={batch_size} trade_cap={trade_cap} raw_limit={raw_limit} history_limit={history_limit}",
            flush=True,
        )

        raw7 = await _fetch_raw_trades(session, since7, raw_limit)
        raw30 = await _fetch_raw_trades(session, since30, raw_limit)
        raw90 = await _fetch_raw_trades(session, since90, raw_limit)
        ...
        for i in range(0, len(wallets), batch_size):
            ...
            h7 = await _fetch_history_rows(session, since7, batch, history_limit)
            h30 = await _fetch_history_rows(session, since30, batch, history_limit)
            h90 = await _fetch_history_rows(session, since90, batch, history_limit)
            ...
            age_rows = (
                await session.execute(select(Wallet.address, Wallet.first_seen_at).where(Wallet.address.in_(batch)))
            ).all()
            ...
            print(
                f"batch {i // batch_size + 1} wallets {i + 1}-{min(i + batch_size, len(wallets))}/{len(wallets)} updated={total_updated}",
                flush=True,
            )

    print(f"✅ recompute_whale_stats_recent completed. Updated {total_updated} wallets.")
    return 0
